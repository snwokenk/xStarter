...
describe("xStarterPoolPairB WITH project token deployed by poolpair and ERC20 funding token", function(){
...
describe('contributeERC20', function() {
    ...
    it('wait till open, contribute and check balance has changed', async function(){
        ...
        await response.wait()
        response = await poolPair.connect(addr1).contributeFundingToken();
        await response.wait();
        ...
        // second contribution
        response = await fundingTokenInst.connect(addr2).approve(poolPair.address, utils.parseEther('1.0'))
        await response.wait()
        response = await poolPair.connect(addr2).contributeFundingToken();
        await response.wait();
        ...
        response = await fundingTokenInst.connect(addr3).approve(poolPair.address, utils.parseEther('1.0'))
        await response.wait()

        await expect(poolPair.connect(addr3).contributeFundingToken()).to.be.revertedWith("revert ILO event not open")
...
describe('createLiquidityPool', function() {
  ...
  it('liquidity pool address should be same on pool pair and uniswap* factory', async function(){
    ...
    let projectTokenAddr = await poolPair.projectToken()
    let fundingTokenAddr = await poolPair.fundingToken()

    let lpAddr1 = await routerFactoryInst.getPair(fundingTokenAddr, projectTokenAddr)
...
describe("xStarterPoolPairB WITH project token deployed by admin and ERC20 funding token", function(){
  const uniswapRouter = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";
  const uniswapFactory = "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f";
  const WETH = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";
  const zeroAddress = "0x0000000000000000000000000000000000000000";
  ...
  describe("Deployment", function () {
    ...
    // If the callback function is async, Mocha will `await` it.
    it("deployed token should have right amount", async function () {
      // Expect receives a value, and wraps it in an Assertion object. These
      // objects have a lot of utility methods to assert values.

      // This test expects total supply of token to equal 500 million 
        expect(await projectTokenInst.totalSupply()).to.equal(BigNumber.from('500000000000000000000000000'))
    });
  ...
  describe("setUpPoolPair Transaction", function () {
    ...
    it("setUpPoolPair Should be setup contract when admin calls", async function () {
        ...
        let startTime = (parseInt(Date.now() / 1000) + startTimeLen);
        let endTime = (parseInt(Date.now() / 1000) + endTimeLen);
        console.log('projecttoken address is', projectTokenInst.address)
        const response = await poolPair.setUpPoolPair(
            projectTokenInst.address,
            "xyz",
            "xyz",
  ...
  describe('depositAllToken', function() {
      ...
      it('should revert because admin has not yet called allowance', async function(){
        await expect(poolPair.depositAllTokenSupply()).to.be.revertedWith("revert You must deposit all available tokens by calling the approve function on the token contract");
      })

      it('should deposit after approving allowance', async function(){
        let response = await projectTokenInst.approve(poolPair.address, '500000000000000000000000000')
        await response.wait()
        response = await projectTokenInst.allowance(owner.address,poolPair.address)
        console.log('response is', response)
        expect(response).to.equal(BigNumber.from('500000000000000000000000000'))
        const depositing = await poolPair.depositAllTokenSupply()
...
describe("xStarterPoolPairB WITH project token deployed by admin and native", function(){
  const uniswapRouter = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";
  const uniswapFactory = "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f";
  const WETH = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";
  const zeroAddress = "0x0000000000000000000000000000000000000000";
...
describe("xStarterPoolPairB WITH token deployed by poolpair and native funding token", function(){
   const uniswapRouter = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";
   const uniswapFactory = "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f";
   const WETH = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";
   const zeroAddress = "0x0000000000000000000000000000000000000000";
