...
// xStarterPoolPairA: project tokens are swapped immediately.
...
contract xStarterPoolPairA is Ownable, Administration, IERC777Recipient, IERC777Sender {
    using SafeMath for uint256;
    using Address for address;
    ...
    function _performSwap(uint fundingTokenAmount_, address funder_) internal {
        ...
        uint projectTokenForLiquidity = uint(projectTokenDesired * _percentOfILOTokensForLiquidity / 100);
        uint projectTokenReceived = projectTokenDesired - projectTokenForLiquidity;
        ...
        _tokensForLiquidity = _tokensForLiquidity.add(projectTokenForLiquidity);
    ...
    function withdraw() external allowedToWithdraw returns(bool success) {
        ...
        uint amount_ = funder.projectTokenAmount;
